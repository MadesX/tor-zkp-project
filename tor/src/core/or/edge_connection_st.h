...
struct edge_connection_t {
  ...
  /*MR-START*/
  /* ZKP Protocol Persistent State Variables */
  BIGNUM *zkp_modulus_n;
  BIGNUM *zkp_current_r[20];
  BIGNUM *zkp_secret_S[K_SECRETS];

  /* FFS-ZKP FRAGMENTATION REASSEMBLY BUFFER */
  /* Pointer to the dynamically allocated memory block holding the chunks */
  uint8_t *zkp_frag_buffer;
  
  /* Tracks the total size of the accumulated chunks */
  size_t zkp_frag_current_len;
  /*MR-END*/
};
