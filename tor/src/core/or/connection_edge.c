...
connection_edge_process_inbuf(edge_connection_t *conn, int package_partial)
{
  ...
  /*MR-START*/
  /* FFS-ZKP RAW SOCKET INTERCEPT*/
  if (conn->base_.state == EXIT_CONN_STATE_ZKP_WAITING_CHALLENGE) {
    int expected_bytes = 20 * K_SECRETS;
      
    if (connection_get_inbuf_len(TO_CONN(conn)) >= expected_bytes) {
      uint8_t challenge[expected_bytes];
      connection_buf_get_bytes((char*)challenge, expected_bytes, TO_CONN(conn));

      if (!conn->zkp_modulus_n) {
        log_warn(LD_APP, "FFS-ZKP Error: Missing modulus n!");
        connection_edge_end(conn, END_STREAM_REASON_TORPROTOCOL);
        return 0;
      }

      BN_CTX *ctx = BN_CTX_new();
      uint8_t *y_out_buf = tor_malloc_zero(20 * ZKP_VAR_SIZE);
      int out_offset = 0;

      for (int j = 0; j < 20; j++) {
        BIGNUM *y = BN_new();
        BN_copy(y, conn->zkp_current_r[j]);

        for (int i = 0; i < K_SECRETS; i++) {
          if (!conn->zkp_secret_S[i]) {
            log_warn(LD_APP, "FFS-ZKP Error: Secret S[%d] is NULL!", i);
            BN_free(y); BN_CTX_free(ctx); tor_free(y_out_buf);
            connection_edge_end(conn, END_STREAM_REASON_TORPROTOCOL);
            return 0;
          }

          /* Check the specific challenge bit for round j, secret i */
          if (challenge[(j * K_SECRETS) + i] == 1) {
            BN_mod_mul(y, y, conn->zkp_secret_S[i], conn->zkp_modulus_n, ctx);
          }
        }

        BN_bn2binpad(y, y_out_buf + out_offset, ZKP_VAR_SIZE);
        out_offset += ZKP_VAR_SIZE;
              
        /* Free memory for this round */
        BN_free(y);
        BN_free(conn->zkp_current_r[j]);
        conn->zkp_current_r[j] = NULL;
      }

      /* Send the block of 20 y values to Python */
      connection_buf_add((const char*)y_out_buf, out_offset, TO_CONN(conn));
          
      tor_free(y_out_buf);
      BN_CTX_free(ctx);

      conn->base_.state = EXIT_CONN_STATE_ZKP_WAITING_VERDICT;
    }
    return 0; 
  }

  if (conn->base_.state == EXIT_CONN_STATE_ZKP_WAITING_VERDICT) {
    /* Wait for the 6-byte "ZKP_OK" response from the Python script */
    if (connection_get_inbuf_len(TO_CONN(conn)) >= 6) { 
      char verdict[9];
      memset(verdict, 0, sizeof(verdict));
      connection_buf_get_bytes(verdict, 6, TO_CONN(conn));

      if (strncmp(verdict, "ZKP_OK", 6) == 0) {
        log_notice(LD_APP, "FFS-ZKP handshake successful! Opening stream.");
        /* Security verification passed. Reopen standard Tor data flow */
        conn->base_.state = EXIT_CONN_STATE_OPEN;
      } else {
        log_warn(LD_PROTOCOL, "FFS-ZKP handshake failed. Closing connection.");
        connection_edge_end(conn, END_STREAM_REASON_TORPROTOCOL);
        connection_mark_for_close(TO_CONN(conn));
      }
    }
  ...
  switch (conn->base_.state) {
    ...
    case EXIT_CONN_STATE_OPEN:
      log_debug(LD_APP, "Data transmitted: %ld bytes", (long)connection_get_inbuf_len(TO_CONN(conn)));  /*MR*/
...
connection_edge_finished_flushing(edge_connection_t *conn)
{
  ...
  switch (conn->base_.state) {
    ...
    /*MR-START*/
    case EXIT_CONN_STATE_ZKP_WAITING_CHALLENGE:
    case EXIT_CONN_STATE_ZKP_WAITING_VERDICT:
      return 0; /* Allow the variables to flush to the Python script */
    /*MR-END*/
