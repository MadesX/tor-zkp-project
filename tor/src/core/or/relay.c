...
/*MR-START*/
static int send_zkp_init_cell(edge_connection_t *conn);
static int send_zkp_init_fragmented(edge_connection_t *conn, const uint8_t *payload, size_t total_len);
/*MR-END*/
    ...
    edge_connection_t *conn, crypt_path_t *layer_hint)
{
  ...
  if (conn->base_.type == CONN_TYPE_AP &&
    ...
    /*MR-START*/
    /* INSERT FFS-ZKP CLIENT TRIGGER */
    if (!entry_conn->use_begindir) {
      if (send_zkp_init_cell(conn) < 0) {
        log_warn(LD_APP, "Failed to send FFS-ZKP init cell.");
      }
    }
    /*MR-END*/
...
/* FFS-ZKP CLIENT INIT CELL SENDER */
static int send_zkp_init_cell(edge_connection_t *conn) {
  BIGNUM *p = BN_new();
  BIGNUM *q = BN_new();
  BIGNUM *n = BN_new();
  ...
  /* Prepare Payload for ONLY S_i and v_i */
  int zkp_out_len = K_SECRETS * 2 * ZKP_VAR_SIZE;
  uint8_t *zkp_out_buf = tor_malloc_zero(zkp_out_len);
  int offset = 0;

  /* Arrays to hold variables for local mathematical verification */
  BIGNUM *S[K_SECRETS] = {NULL};
  BIGNUM *v[K_SECRETS] = {NULL};
  BIGNUM *D[K_SECRETS] = {NULL};
  BIGNUM *sqr = BN_new();

  /* Generate Secrets and verify they will perfectly yield 'n' */
  int valid_matrix = 0;
  while (!valid_matrix) {
    for (int i = 0; i < K_SECRETS; i++) {
      /* Allocate memory only if it hasn't been allocated yet */
      if (!S[i]) S[i] = BN_new();
      if (!v[i]) v[i] = BN_new();
      if (!D[i]) D[i] = BN_new();

      /* Generate S_i coprime to n */
      do {
        BN_rand_range(S[i], n);
        BN_gcd(gcd, S[i], n, ctx);
      } while (!BN_is_one(gcd));

      /* Calculate v_i = S_i^2 (mod n) */
      BN_mod_sqr(v[i], S[i], n, ctx);

      /* Calculate D_i = S_i^2 - v_i */
      BN_sqr(sqr, S[i], ctx);
      BN_sub(D[i], sqr, v[i]);
    }

    /* Test if the GCD of the first two pairs is exactly n */
    if (K_SECRETS >= 2) {
      BIGNUM *test_n = BN_new();
      BN_gcd(test_n, D[0], D[1], ctx);
            
      if (BN_cmp(test_n, n) == 0) {
        valid_matrix = 1; /* The math is perfect. Break the loop. */
      }
      BN_free(test_n);
    } else {
      valid_matrix = 1; /* Safety fallback */
    }
  }

  /* Write the mathematically validated S_i and v_i to the buffer */
  for (int i = 0; i < K_SECRETS; i++) {
    BN_bn2binpad(S[i], zkp_out_buf + offset, ZKP_VAR_SIZE);
    offset += ZKP_VAR_SIZE;
        
    BN_bn2binpad(v[i], zkp_out_buf + offset, ZKP_VAR_SIZE);
    offset += ZKP_VAR_SIZE;
        
    /* Clean up loop memory */
    BN_free(S[i]); BN_free(v[i]); BN_free(D[i]);
  }

  /* Pass the payload to the Fragmentation Engine */
  send_zkp_init_fragmented(conn, zkp_out_buf, zkp_out_len);

  /* Final Memory Cleanup */
  tor_free(zkp_out_buf);
  BN_free(p); BN_free(q); BN_free(n); 
  BN_free(gcd); BN_free(sqr);
  BN_CTX_free(ctx);

  return 0;
}

/* Helper function to slice and send large payloads across the Tor network */
static int send_zkp_init_fragmented(edge_connection_t *conn, const uint8_t *payload, size_t total_len) {
  size_t offset = 0;

  /* Phase 1: Always send the START chunk */ 
  size_t start_chunk = (total_len > RELAY_PAYLOAD_SIZE) ? RELAY_PAYLOAD_SIZE : (total_len - 1);
  connection_edge_send_command(conn, RELAY_COMMAND_ZKP_FRAG_START, (const char *)payload, start_chunk);
  offset += start_chunk;

  /* Phase 2: Send intermediate CONT chunks (Only triggers if payload is massive) */
  while (total_len - offset > RELAY_PAYLOAD_SIZE) {
    connection_edge_send_command(conn, RELAY_COMMAND_ZKP_FRAG_CONT, (const char *)(payload + offset), RELAY_PAYLOAD_SIZE);
    offset += RELAY_PAYLOAD_SIZE;
  }

  /* Phase 3: Always send an END chunk (guaranteed to be > 0 bytes) */
  size_t end_chunk = total_len - offset;
  connection_edge_send_command(conn, RELAY_COMMAND_ZKP_FRAG_END, (const char *)(payload + offset), end_chunk);

  return 0;
}

/* Extracts the challenge bit a_i from the Service Provider's cell payload */
static int extract_challenge_bit(const uint8_t *payload, int index) {
  /* Tor cell data begins after the relay header */
  int offset = RELAY_HEADER_SIZE;
    
  /* We assume the Service Provider sends an array of bytes where 1 = 1, and 0 = -1 */
  uint8_t bit = payload[offset + index];
  return (bit == 1) ? 1 : -1;
}

/* Packages the OpenSSL BIGNUM 'y' into a Tor cell and sends it backward down the circuit */
static void send_zkp_response_cell(edge_connection_t *conn, BIGNUM *y) {
  uint8_t payload[RELAY_PAYLOAD_SIZE];
  memset(payload, 0, sizeof(payload));

  /* Convert BIGNUM y back to a raw binary array */
  int y_len = BN_num_bytes(y);
    
  if (y_len <= RELAY_PAYLOAD_SIZE) {
    BN_bn2bin(y, payload);
  } else {
    log_warn(LD_PROTOCOL, "FFS-ZKP: BIGNUM y is too large for a Tor cell!");
    return;
  }

  /* Send the cell backward toward the Service Provider */
  connection_edge_send_command(conn, RELAY_COMMAND_ZKP_RESPONSE, (char*)payload, y_len);
}

// Static variables for replay attack simulation
static uint8_t *cached_zkp_payload = NULL;
...
//  log_fn(LOG_DEBUG,"command %d stream %d", rh.command, rh.stream_id);
  ...
  /*MR-START*/
  /* FFS-ZKP INTERCEPT LOGIC*/
  /* Ensure conn is not NULL before checking connection state variables */
  if (conn) {
    /* Intercept the custom ZKP fragmentation cell types */
    if (rh.command == RELAY_COMMAND_ZKP_FRAG_START ||
        rh.command == RELAY_COMMAND_ZKP_FRAG_CONT ||
        rh.command == RELAY_COMMAND_ZKP_FRAG_END) {
          
      edge_connection_t *edge_conn = conn;

      /* Phase 1: Initialize the buffer on the START chunk */
      if (rh.command == RELAY_COMMAND_ZKP_FRAG_START) {
        log_warn(LD_APP, "FFS-ZKP: Received START chunk (%d bytes).", rh.length);
              
        if (edge_conn->zkp_frag_buffer) {
          tor_free(edge_conn->zkp_frag_buffer); 
        }
        edge_conn->zkp_frag_buffer = tor_malloc(rh.length);
        memcpy(edge_conn->zkp_frag_buffer, cell->payload + RELAY_HEADER_SIZE, rh.length);
        edge_conn->zkp_frag_current_len = rh.length;
        return 0; 
      }

      /* Phase 2: Append data on CONTINUATION or END chunks */
      if (rh.command == RELAY_COMMAND_ZKP_FRAG_CONT || rh.command == RELAY_COMMAND_ZKP_FRAG_END) {
        if (!edge_conn->zkp_frag_buffer) {
          log_warn(LD_PROTOCOL, "FFS-ZKP Error: ZKP frag without START cell. Closing.");
          return -END_CIRC_REASON_TORPROTOCOL;
        }

        /* Expand the memory allocation to fit the new chunk */
        edge_conn->zkp_frag_buffer = tor_realloc(edge_conn->zkp_frag_buffer,
                                                 edge_conn->zkp_frag_current_len + rh.length);
              
        /* Copy the new chunk exactly to the end of the current buffer */
        memcpy(edge_conn->zkp_frag_buffer + edge_conn->zkp_frag_current_len,
               cell->payload + RELAY_HEADER_SIZE, rh.length);
                     
        edge_conn->zkp_frag_current_len += rh.length;
      }

      /* Phase 3: Execute the cryptographic logic when the final chunk arrives */
      if (rh.command == RELAY_COMMAND_ZKP_FRAG_END) {
        log_warn(LD_APP, "FFS-ZKP: Received END chunk. Buffer complete: %d bytes. Executing Verification", (int)edge_conn->zkp_frag_current_len);

        // MALICIOUS INJECTION Part 1 START
        char *malicious_env = getenv("IS_MALICIOUS");
        if (malicious_env != NULL && strcmp(malicious_env, "1") == 0) {
          log_warn(LD_APP, "MaliciousExit: Sabotage Detected");

          // Send zeros for the entire buffer
          int zkp_out_len = ZKP_VAR_SIZE + (K_SECRETS * ZKP_VAR_SIZE) + (20 * ZKP_VAR_SIZE);
          uint8_t *zkp_out_buf = tor_malloc_zero(zkp_out_len);
            
          connection_buf_add((const char*)zkp_out_buf, zkp_out_len, TO_CONN(edge_conn));
          tor_free(zkp_out_buf);
            
          // Clean up and exit the function early
          tor_free(edge_conn->zkp_frag_buffer);
          edge_conn->zkp_frag_buffer = NULL;
          return 0;
        }
        // MALICIOUS INJECTION Part 1 END
              
        int offset = 0;
        BN_CTX *ctx = BN_CTX_new();
        BIGNUM *v_array[K_SECRETS];

        /* Extract Secrets (S_i) and Public Keys (v_i) from the fragmented buffer */
        for (int i = 0; i < K_SECRETS; i++) {
          edge_conn->zkp_secret_S[i] = BN_bin2bn(edge_conn->zkp_frag_buffer + offset, ZKP_VAR_SIZE, NULL);
          offset += ZKP_VAR_SIZE;
                  
          v_array[i] = BN_bin2bn(edge_conn->zkp_frag_buffer + offset, ZKP_VAR_SIZE, NULL);
          offset += ZKP_VAR_SIZE;
        }

        /* Extract Modulus n using GCD of the first two pairs */
        if (K_SECRETS >= 2) {
          BIGNUM *D1 = BN_new();
          BIGNUM *D2 = BN_new();
          BIGNUM *sqr = BN_new();
          edge_conn->zkp_modulus_n = BN_new();

          /* D1 = (S_0)^2 - v_0 */
          BN_sqr(sqr, edge_conn->zkp_secret_S[0], ctx);
          BN_sub(D1, sqr, v_array[0]);

          /* D2 = (S_1)^2 - v_1 */
          BN_sqr(sqr, edge_conn->zkp_secret_S[1], ctx);
          BN_sub(D2, sqr, v_array[1]);

          /* Reverse-engineer n = GCD(D1, D2) */
          BN_gcd(edge_conn->zkp_modulus_n, D1, D2, ctx);

          BN_free(D1); BN_free(D2); BN_free(sqr);
          log_warn(LD_APP, "FFS-ZKP: Modulus n successfully reverse-engineered via GCD!");
        } else {
          log_warn(LD_APP, "FFS-ZKP Error: Need at least 2 secrets to calculate n via GCD.");
          return -END_CIRC_REASON_TORPROTOCOL;
        }

        /* 20-ROUND MULTI-PAYLOAD GENERATION                          */
        BIGNUM *x = BN_new();
        int zkp_out_len = ZKP_VAR_SIZE + (K_SECRETS * ZKP_VAR_SIZE) + (20 * ZKP_VAR_SIZE);
        uint8_t *zkp_out_buf = tor_malloc_zero(zkp_out_len);
        int out_offset = 0;

        /* Write reverse-engineered Modulus n to Python buffer */
        BN_bn2binpad(edge_conn->zkp_modulus_n, zkp_out_buf + out_offset, ZKP_VAR_SIZE);
        out_offset += ZKP_VAR_SIZE;

        /* Write extracted Public Keys v_i to Python buffer */
        for (int i = 0; i < K_SECRETS; i++) {
          BN_bn2binpad(v_array[i], zkp_out_buf + out_offset, ZKP_VAR_SIZE);
          out_offset += ZKP_VAR_SIZE;
          BN_free(v_array[i]); /* Free local v variable */
        }

        /* Generate 20 rounds of Commitments (x) */
        for (int j = 0; j < 20; j++) {
          edge_conn->zkp_current_r[j] = BN_new();
          BN_rand_range(edge_conn->zkp_current_r[j], edge_conn->zkp_modulus_n);
                  
          int sign = (crypto_rand_int(2) == 0) ? 1 : -1;
          BN_mod_sqr(x, edge_conn->zkp_current_r[j], edge_conn->zkp_modulus_n, ctx);
                  
          if (sign == -1) {
            BN_mod_sub(x, edge_conn->zkp_modulus_n, x, edge_conn->zkp_modulus_n, ctx);
          }
                  
          BN_bn2binpad(x, zkp_out_buf + out_offset, ZKP_VAR_SIZE);
          out_offset += ZKP_VAR_SIZE;
        }

        // MALICIOUS INJECTION Part 2 START
        if (malicious_env != NULL && strcmp(malicious_env, "2") == 0) {
          log_warn(LD_APP, "Exit: Executing Replay");

          if (cached_zkp_payload == NULL) {
            // First time: Act honest, but save the proof
            log_warn(LD_APP, "Exit: Caching valid proof for replay");

            cached_zkp_payload = tor_memdup(zkp_out_buf, zkp_out_len);
            cached_len = zkp_out_len;
          } else {
            // Second time: REPLAY the cached proof
            log_warn(LD_APP, "MaliciousExit: Replaying Detected");
            connection_buf_add((const char*)cached_zkp_payload, cached_len, TO_CONN(edge_conn));
            return 0;
          }
        } else if (malicious_env != NULL && strcmp(malicious_env, "3") == 0) {
          log_warn(LD_APP, "MaliciousExit: Modification Detected");
          // Flip bits in the commitment (e.g., the 5th byte of the payload)
          zkp_out_buf[ZKP_VAR_SIZE + (K_SECRETS * ZKP_VAR_SIZE) + 5] ^= 0xFF;
        }
        // MALICIOUS INJECTION Part 2 END

        /* Send raw bytes directly to the Python Service Provider via TCP */
        connection_buf_add((const char*)zkp_out_buf, zkp_out_len, TO_CONN(edge_conn));
        tor_free(zkp_out_buf);

        /* Change state to wait for the Python script's challenge */
        conn->base_.state = EXIT_CONN_STATE_ZKP_WAITING_CHALLENGE;
              
        BN_free(x);
        BN_CTX_free(ctx);

        /* Clean up the reassembly buffer once everything is handled */
        tor_free(edge_conn->zkp_frag_buffer);
        edge_conn->zkp_frag_buffer = NULL;
        edge_conn->zkp_frag_current_len = 0;
      }
