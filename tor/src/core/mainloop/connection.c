...
connection_free_minimal(connection_t *conn)
{
  ...
  /*MR-START*/
  /* Free ZKP parameters safely before the connection struct is wiped or freed */
  if (conn->type == CONN_TYPE_EXIT) {
    edge_connection_t *edge_conn = TO_EDGE_CONN(conn);

    /* Free the fragmentation reassembly buffer */
    ...
    /* Free the Modulus */
    if (edge_conn->zkp_modulus_n) {
      BN_free(edge_conn->zkp_modulus_n);
      edge_conn->zkp_modulus_n = NULL;
    }
      
    /* Free the 20-round array of 'r' values */
    for (int j = 0; j < 20; j++) {
      if (edge_conn->zkp_current_r[j]) {
        BN_free(edge_conn->zkp_current_r[j]);
        edge_conn->zkp_current_r[j] = NULL;
      }
    }
      
    /* Free the Secrets array */
    for (int i = 0; i < K_SECRETS; i++) { 
      if (edge_conn->zkp_secret_S[i]) {
        BN_free(edge_conn->zkp_secret_S[i]);
        edge_conn->zkp_secret_S[i] = NULL;
      }
    }
  }
  /*MR-END*/
